...
import { InternalEvents, InternalFightEvents, InternalGameEvents, InternalRoomEvents, InternalStatsEvents, InternalTurnEvents } from '@app/constants/internal-events';
import { FIGHT_TURN_DURATION_IN_S, FIGHT_TURN_DURATION_NO_FLEE_IN_S, MIN_VP_TURN_DELAY, MOVEMENT_TIMEOUT_IN_MS, ONE_SECOND_IN_MS, TimerType } from '@app/gateways/game/game.gateway.constants';
    ...
    describe('Tests spécifiques pour les méthodes demandées', () => {
        ...
        describe('processVirtualPlayerInstructions', () => {
            ...
            it('devrait appeler processPath quand action est Move', () => {
                // Arrange
                const pathInfo = { path: [{ x: 1, y: 1 }, { x: 2, y: 2 }], cost: 2 };
                const instruction = {
...
expect(game.endTurn).toHaveBeenCalled();
            ...
            it('devrait s\'arrêter après avoir initié un combat', () => {
                ...
                jest.spyOn(VPManager, 'computePath')
                    .mockReturnValueOnce(moveInstruction)
                    .mockReturnValueOnce(fightInstruction);
